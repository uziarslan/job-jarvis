...
const injectJobApplyButton = async () => {
  ...
  const createButton = (className: string, jobData: any, textareaSelector: any) => {
    const button = document.createElement('a');
    button.className = `my-5 ${className}`;
    button.innerHTML = `
      <div>
    ...
    button.addEventListener('click', (e) => {
      ...
      renderModal(modalRoot!, {
        ...
        title: jobData.type === 'coverLetter' ? 'Generate Cover Letter' : 'Generate Answer',
        jobData
      });
    });
    return button;
  ...
  const getJobData = async (questionText?: string) => {
    ...
    const jobSkills = jobSkillsContainer
      ? Array.from(jobSkillsContainer.querySelectorAll('li span'))
    ...
    return {
      ...
      jobSkills,
      jobPosted,
      type: questionText ? 'question' : 'coverLetter',
      ...(questionText && { questionText })
    };
  };

  let modalRoot = document.getElementById('job-jarvis-modal-root') || createModalRoot();
  ...
  const findAndInjectButton = async () => {
    ...
    if (coverLetterArea && !coverLetterArea.querySelector('.upsale-button')) {
      ...
      const button = createButton('upsale-button', jobData, textarea);
      coverLetterArea.firstElementChild?.appendChild(button);
    }

    if (questionsArea) {
      Array.from(questionsArea.children).forEach(async (questionElement) => {
        if (!questionElement.querySelector('.job-jarvis-button')) {
          ...
          const jobData = await getJobData(questionText);
          const textarea = questionElement.querySelector('div.air3-textarea.textarea-wrapper.p-0 > textarea');
          const button = createButton('job-jarvis-button', jobData, textarea);
          questionElement.appendChild(button);
        }
      });
    }

    if (!coverLetterArea && !questionsArea && attempts++ < maxAttempts) {
      setTimeout(findAndInjectButton, 500);
    }
  };
